...
\end{equation}      
Here, $\phi(\mu,\ \sigma^2)(y)$ is the PDF of the normal distribution.  Setting $\pi = 0.3$ and $\sigma^2 = 1$ produces the following distribution for $n = 10000$:
...
The Q-Q plot shows that the assumption of homogeneous variance for the error distribution would not be valid in this situation [@Dave; @Schork; @Everitt; @Pears].     
...
\begin{equation}    
{\mathbb{P}}_{i} = \{{P}_{i}({y}_{i(1)}),\ {P}_{i}({y}_{i(2)}),\ ...,\ {P}_{i}({y}_{i(r_{i})})\}, (\#eq:System4)
\end{equation}    
where ${P}_{i}({y}_{i(r_{i})}) = 1-\sum_{h=1}^{r_{i}-1}{P}_{i}({y}_{i(h)})$, and cumulative probabilities:    
\begin{equation}   
{\mathbb{F}}_{i} = \{{F}_{i}({y}_{i(l)}) = \sum_{j=1}^{l}{P}_{i}({y}_{i(j)}),\ l = 1,\ ...,\ r_{i}-1\}. (\#eq:System5)
\end{equation}     
